	...
	<div class="fixed inset-0 z-50 md:hidden">
		...
		<div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[85vh] overflow-y-auto">
			...
			<div class="p-4 space-y-2">
					...
					{#if expandedSections.estado}
						<div class="flex flex-wrap gap-2 pb-3">
							{#each [['Obra nueva', 'nueva'], ['Buen estado', 'bueno'], ['A reformar', 'reformar']] as [label, val]}
								<button on:click={() => { setEstadoValue(val); syncFiltersToUrl(); }}
									class="px-3 py-1.5 rounded-full text-sm {$filters.estado === val ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}">
									{label}
								</button>
							{/each}
						</div>
				...
				<!-- Equipamiento -->
				<div>
					<button on:click={() => toggleSection('equipamiento')} class="w-full flex items-center justify-between py-2 text-sm font-medium text-gray-900">
						Equipamiento
						<ChevronDown class="w-4 h-4 text-gray-400 transition-transform {expandedSections.equipamiento ? 'rotate-180' : ''}" />
					</button>
					{#if expandedSections.equipamiento}
						<div class="flex flex-wrap gap-2 pb-3">
							{#each [['Amueblado', 'amueblado'], ['Solo cocina equipada', 'cocina-equipada']] as [label, val]}
								<button on:click={() => setEquipamientoValue(val)}
									class="px-3 py-1.5 rounded-full text-sm {$filters.equipamiento === val ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}">
									{label}
								</button>
							{/each}
						</div>
					...
					<div class="pb-3">
						...
						{:else if section.key === 'equipamiento'}
							<div class="space-y-2">
								{#each [['Amueblado', 'amueblado'], ['Solo cocina equipada', 'cocina-equipada']] as [label, val]}
									<label class="flex items-center gap-3 cursor-pointer group">
										<input type="radio" name="equipamiento" checked={$filters.equipamiento === val} on:change={() => setEquipamientoValue(val)} class="w-4 h-4 text-primary" />
										<span class="text-sm text-gray-600 group-hover:text-gray-900">{label}</span>
									</label>
								{/each}
							</div>
