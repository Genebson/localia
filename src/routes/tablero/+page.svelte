<script lang="ts">
	import { Search as SearchIcon, Phone, MapPin, Clock, Plus, X, Check, User } from 'lucide-svelte';
	...
	const propertyTypes = ['Casa', 'Departamento', 'Terreno', 'Galpón', 'Local', 'Chacra', 'Quinta', 'Oficina'];
...
<main class="pt-16 md:pt-20 min-h-screen bg-background">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		<div class="flex items-center justify-between mb-8">
			<div>
				<h1 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Buscamos propiedades</h1>
				<p class="text-gray-500">Encontrá compradores que buscan exactamente lo que tenés</p>
			</div>
		<button
			on:click={openForm}
		...
		<div class="mb-6 flex flex-wrap gap-2">
			<button
				...
				class="px-4 py-2 rounded-full text-sm font-medium transition-colors {selectedType === '' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
			>
				Todas
			</button>
			{#each propertyTypes as type}
				<button
					...
					class="px-4 py-2 rounded-full text-sm font-medium transition-colors {selectedType === type ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
				>
					{type}
				</button>
			{/each}
				...
				<div class="bg-white rounded-xl shadow-sm p-5 hover:shadow-md transition-shadow">
					<div class="flex items-start gap-3 mb-4">
						<div class="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
							<span class="text-primary font-semibold">{getInitials(search.name)}</span>
						</div>
						<div class="flex-1 min-w-0">
							<h3 class="font-semibold text-gray-900 truncate">{search.name}</h3>
							<div class="flex items-center gap-1 text-sm text-gray-500">
								<Clock class="w-3 h-3" />
								<span>Hace {search.timestamp} {search.timestamp === 1 ? 'día' : 'días'}</span>
							</div>
						</div>
					</div>
					...
					<div class="space-y-2 mb-4">
						<div class="flex items-center gap-2 text-sm">
							<span class="px-2 py-0.5 bg-primary/10 text-primary rounded font-medium">{search.type}</span>
	...
	{#if showForm}
		<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
			<div class="absolute inset-0 bg-black/50" on:click={() => showForm = false} on:keydown={(e) => e.key === 'Escape' && (showForm = false)} role="button" tabindex="0" aria-label="Cerrar"></div>
			<div class="relative bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
				<button on:click={() => showForm = false} class="absolute top-4 right-4 p-1 hover:bg-gray-100 rounded-full">
				...
				<form on:submit|preventDefault={handleSubmit} class="space-y-4">
					<div>
						...
						<input type="text" bind:value={formData.name} required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
					</div>
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1">Teléfono de contacto</label>
						<input type="tel" bind:value={formData.phone} required placeholder="Ej: 5491123456789" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
					</div>
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1">Tipo de propiedad</label>
						<select bind:value={formData.type} required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none">
					...
					<div>
						...
						<input type="text" bind:value={formData.zone} required placeholder="Ej: Palermo, Buenos Aires" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
					</div>
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1">Presupuesto</label>
						<input type="text" bind:value={formData.budget} required placeholder="Ej: USD 200.000" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
					</div>
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1">Mensaje</label>
						<textarea bind:value={formData.message} rows="3" required placeholder="Describe qué estás buscando..." class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none resize-none"></textarea>
					</div>
