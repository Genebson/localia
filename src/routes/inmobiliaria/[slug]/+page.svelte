...
<main class="min-h-screen bg-background">
	{#if !inmob}
		<div class="max-w-7xl mx-auto px-4 py-16 text-center">
			...
			<a href="{base}/" class="px-6 py-3 bg-primary text-white font-semibold rounded-lg"
				>Volver al inicio</a
			>
		</div>
	{:else}
			...
			<div class="absolute bottom-0 left-0 right-0 p-6 md:p-8">
				<div class="max-w-7xl mx-auto flex items-end gap-4">
					<img
						src={inmob.logo}
						alt={inmob.name}
						class="w-20 h-20 md:w-24 md:h-24 rounded-xl bg-white p-1 shadow-lg"
					/>
				...
				<div class="lg:col-span-1">
					<div class="bg-white rounded-xl shadow-sm p-6 sticky top-24">
						...
						{#if $isAgent}
							<a
								href="{base}/publicar"
								class="flex items-center justify-center gap-2 w-full px-4 py-3 bg-accent hover:bg-accent-hover text-white font-semibold rounded-lg transition-colors mb-4"
							>
								<Plus class="w-5 h-5" />
								Publicar propiedad
							</a>
						...
						<div class="space-y-3">
							<a
								href="tel:{inmob.phone}"
								class="flex items-center gap-2 w-full px-4 py-3 border border-gray-200 hover:border-primary hover:bg-primary/5 rounded-lg transition-colors"
							>
								<Phone class="w-5 h-5 text-gray-400" />
								<span class="text-gray-700">Llamar</span>
							</a>
							<a
								href="mailto:{inmob.email}"
								class="flex items-center gap-2 w-full px-4 py-3 border border-gray-200 hover:border-primary hover:bg-primary/5 rounded-lg transition-colors"
							>
