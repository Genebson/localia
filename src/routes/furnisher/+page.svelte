...
<main class="pt-16 md:pt-20 min-h-screen bg-background">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		<div class="text-center mb-12">
			<div class="inline-flex items-center gap-2 px-4 py-2 bg-purple-100 text-purple-700 rounded-full text-sm font-medium mb-4">
				<Sparkles class="w-4 h-4" />
				Powered by Furnisher.ai
			</div>
			<h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Mejorá tus fotos con IA</h1>
			<p class="text-gray-500 text-lg max-w-2xl mx-auto">
				Transformá propiedades vacías o desordenadas en imágenes profesionales amuebladas con inteligencia artificial
			</p>
		</div>
		...
		<div class="mb-12">
			...
			<div class="flex flex-wrap gap-3">
				{#each styles as style}
					<span class="px-4 py-2 bg-gray-100 text-gray-700 rounded-full text-sm font-medium">{style}</span>
				{/each}
			</div>
		</div>
			...
			{#each examples as example}
				<div class="bg-white rounded-xl shadow-sm p-6">
					...
					/>
						...
						<span class="absolute top-4 left-4 px-3 py-1 bg-black/60 text-white text-sm font-medium rounded-full">
							{example.beforeLabel}
						</span>
						<span class="absolute top-4 right-4 px-3 py-1 bg-black/60 text-white text-sm font-medium rounded-full">
							{example.afterLabel}
						</span>
					</div>
				</div>
		...
		<div class="mt-12 p-6 bg-gradient-to-r from-primary/5 to-accent/5 rounded-xl text-center">
			...
			<h3 class="text-lg font-semibold text-gray-900 mb-2">¿Querés mejorar tus propiedades?</h3>
			<p class="text-gray-500 mb-4">Subí tus fotos y lasciá que la IA las transforme</p>
			<button class="px-6 py-3 bg-primary hover:bg-primary-light text-white font-semibold rounded-lg transition-colors">
				Subir fotos
			</button>
		</div>
	</div>
