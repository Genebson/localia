		...
		<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
			...
			>
				<div>
					...
					<p class="text-gray-500 text-sm sm:text-base">
						Asistente de WhatsApp para inmobiliarias
					</p>
				</div>
				<div
					class="flex items-center gap-2 px-3 py-1.5 bg-green-100 text-green-700 rounded-full text-sm font-medium w-fit"
				>
			...
			>
				<div class="grid grid-cols-1 md:grid-cols-3 h-full">
					<div
						class="border-r border-gray-100 overflow-y-auto {mobileView === 'list'
							? 'flex'
						...
						{:else}
							{#each conversations as conv}
								<button
									...
									class="w-full text-left p-3 sm:p-4 border-b border-gray-50 hover:bg-gray-50 transition-colors {selectedConversation?.id ===
									conv.id
								...
								>
									<div class="flex items-center gap-2 sm:gap-3">
										<div
											class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0"
										>
					...
					<div
						class="md:col-span-2 flex flex-col {mobileView === 'chat'
							? 'flex'
							: 'hidden md:flex'}"
					>
						<div
							class="p-3 sm:p-4 border-b border-gray-100 flex items-center justify-between gap-2"
						>
							<div class="flex items-center gap-2 sm:gap-3">
								<button
									on:click={() => (mobileView = 'list')}
									class="md:hidden p-1 hover:bg-gray-100 rounded-full shrink-0"
								>
									<svg
								...
								<div
									class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0"
								>
									<Bot class="w-5 h-5 text-primary" />
								</div>
								<div>
						...
						<div class="p-3 sm:p-4 border-t border-gray-100">
							<div class="flex items-center gap-2 sm:gap-3">
								<input
									...
									class="flex-1 px-3 sm:px-4 py-2 sm:py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none text-sm sm:text-base"
								/>
								<button
									on:click={sendMessage}
									disabled={!newMessage.trim() || !selectedConversation}
									class="p-2 sm:p-3 bg-primary hover:bg-primary-light text-white rounded-xl transition-colors disabled:opacity-50 shrink-0"
								>
