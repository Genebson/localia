							...
							{#each selectedConversation.messages as msg}
								<div class="flex {msg.isBot ? 'justify-start' : 'justify-end'}">
									<div class="max-w-[80%] {msg.isBot ? 'order-1' : 'order-2'}">
										<div
											class="flex items-end gap-2 {msg.isBot
												? 'flex-row'
												: 'flex-row-reverse'}"
										>
											<div
												class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 {msg.isBot
													? 'bg-primary/10'
													: 'bg-accent/10'}"
											>
												{#if msg.isBot}
											...
											<div
												class="px-4 py-2.5 rounded-2xl {msg.isBot
										...
										<p
											class="text-xs text-gray-400 mt-1 {msg.isBot
												? 'ml-10'
												: 'mr-10 text-right'}"
										>
											{msg.timestamp}
