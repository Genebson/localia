<script lang="ts">
	...
	let pendingProfileImage: string | null = null;
	...
	async function handlePhotoUpload(event: Event) {
		...
		reader.onload = (e) => {
			pendingProfileImage = e.target?.result as string;
		};
	...
	async function handleProfileSave() {
		...
		if (!hasError && pendingProfileImage) {
			section1Saving = true;
			try {
		...
		if (!hasError && $currentUser.role === 'seeker') {
			section2Saving = true;
			try {
		...
		if (!hasError) {
			...
			editingRentalProfile = false;
			editingIntroLetter = false;
		}
	}
					...
					{#if editingContact}
						<div class="space-y-4">
							...
							<button
								...
								class="w-full py-2 bg-primary text-white font-semibold rounded-lg hover:bg-primary-light transition-colors disabled:opacity-50"
							>
								{section1Saving ? 'Guardando...' : 'Guardar'}
							</button>
						</div>
					{:else}
				...
				{#if $currentUser.role === 'seeker'}
					<div class="bg-white rounded-2xl shadow-sm p-6 mb-8">
						<div class="flex items-center justify-between mb-4">
							...
							>
								<Pencil class="w-4 h-4" />
								{editingRentalProfile ? 'Cancelar' : 'Editar'}
							</button>
						</div>
						{#if editingRentalProfile}
							<div class="space-y-4">
								...
								<button
									on:click={handleProfileSave}
									disabled={section2Saving}
									class="w-full py-2 bg-primary text-white font-semibold rounded-lg hover:bg-primary-light transition-colors disabled:opacity-50"
								>
									{section2Saving ? 'Guardando...' : 'Guardar'}
								</button>
							</div>
						{:else}
							<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						{/if}
							...
							>
								<Pencil class="w-4 h-4" />
								{editingIntroLetter ? 'Cancelar' : 'Editar'}
							</button>
						</div>
						{#if editingIntroLetter}
							<div class="space-y-4">
								<div class="relative">
									<textarea
